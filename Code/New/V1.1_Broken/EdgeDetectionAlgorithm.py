...
def calculatedEdges(primEdgeSystem: ChunkSystem) -> ChunkSystem:
	...
	# print(primEdgeSystem)

	for i in range(DIMENSIONS//CHUNKSIZE):
		for j in range(DIMENSIONS//CHUNKSIZE):
			edgeList = primEdgeSystem.Query(MapTile(j, i), 0)
			for idx in range(len(edgeList)):
				if idx < len(edgeList):
					startingLineSeg = edgeList[idx].edgeData
				else:
					break
				...
				while jdx < len(edgeList):
					addingLineSeg = edgeList[jdx].edgeData
			...
			edgeSystem.insert(edgeChunk(j, i, edgeList))
