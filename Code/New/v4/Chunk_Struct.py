...
class ChunkSystem:
    ...
    def insert(self, point, dataIsArray=False):
        ...
        chunkKey = f'{int(currChunk.x)} {int(currChunk.y)}'
    ...
    def Query(self, point, radius, showChunkKey=False):
        ...
        # Create grid PointOBJ
        currChunk = PointOBJ(point.pos.x // self.cellSize, point.pos.y // self.cellSize)
        if showChunkKey: print(currChunk)

        if radius != 0:  # more efficient for querying single chunk
            for y in range(int(currChunk.y) - radius, int(currChunk.y) + radius + 1):
                for x in range(int(currChunk.x) - radius, int(currChunk.x) + radius + 1):
                    ...
                    chunkKey = f'{x} {y}'
                    if showChunkKey: print(chunkKey)
