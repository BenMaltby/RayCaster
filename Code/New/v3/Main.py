...
RayCastViewButton = GEWY.Button(5, 10, 20, 20, "3D View")
...
SPEED = 1 # if RayCastViewButton.returnState() else 0.5 # Pixels
TORQUE = 2  # Pixels
...
Player = Turret(49, 49, 270, 80, 1, (0, 255, 0))
...
def main():
	...
	ChunkedMap, Map = GenerateChunks("MainMap.png")  # Generates a ChunkSystem of GameBoard Image
	...
	tConstants, numOfRays = thetaOffsetConstants(Player.angleOfVision//Player.step, 227, 340.5)
	...
	while running:  # main game loop
		...
		if RayCastViewButton.returnState():
			...
			sceneB = sceneBrightnessSlider.returnValue() if brightnessSettings else 0.03 # 0.02
		...
		# if Rendering 2D View
			...
			MapProcessing.renderMap2D(screen, Map, Player)  # responsible for drawing scene
			Player.fireRays(screen, tConstants, numOfRays, RayCastViewButton.returnState())  # generates ray coordinates
			DSD = CastRays(screen, Player, edgeChunkSystem, Map, tConstants, RayCastViewButton.returnState())  # compares ray's against walls
