...
class VariableSlider(Object):
	...
	def LeftButtonDown(self):
		...
		if self.vslider.SliderPos.x - self.vslider.sliderW/2 - 2 <= mVec.x <= (self.vslider.SliderPos.x - self.vslider.sliderW/2) + self.vslider.sliderW+2:
...
class Button(Object):
	def __init__(self, x, y, w, h, nt="", state=False):
		super().__init__(x, y, w, h)
		self.state = state
		self.nameTag = nt
	...
	def show(self, screen):
		...
		screen.blit(textsurface,
					(self.pos.x + self.dimensions.x+5, self.pos.y-2))  # 5, -26 are random constants that I think look good.
