...
def CastRays(screen, turret, edgeSystem: ChunkSystem, ChunkRadius, threeDview=True):
	...
	for y in range(len(turret.rayData)):
		...
		for i in range(len(WallData)):
			for j, edge in enumerate(WallData[i].edgeData):
				pointOfIntersection = intersects(playerLignSeg, edge.edgeData)
				if pointOfIntersection:
					currentIntersections.append((pointOfIntersection, edge.edgeData.orientation))
				else:
